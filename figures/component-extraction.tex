...
\end{tikzpicture}
\caption{\centering Diagram of the build-only and build-and-integrate routes: the combined route is shown above, the build-only route below. The dashed lines mark the options to separate the result into single components (build-only route) or merge them into one ordered list (combined route). The $A_3^1$ antenna set is used as an example.}
