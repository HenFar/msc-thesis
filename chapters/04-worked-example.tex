...
\begin{equation}\label{eq:A30}
    ...
    \end{split}
\end{equation}
Note that this result is already fully normalised: the $2g_s^2C_F$ prefactor, together
with the cancellation of the electric charge $e_q^2$ between $|\mathcal M_3^0|^2$ and
$|\mathcal M_2^0|^2$, strips away every coupling-, charge-, and colour-dependent factor
carried by the raw amplitudes above, leaving the purely kinematic result in Eq.~\eqref{eq:A30}.
This is the same normalisation already folded directly into the $|\mathcal M_n^0|^2$
notation used throughout Chapter~\ref{ch:physicsbackground}'s general antenna-construction
formalism (Subsection~\ref{subsec:antFamilies}); the derivation here is kept fully explicit,
coupling and charge included, to show each step by hand.
...
unintegrated antenna in Eq.~\eqref{eq:A30},
\begin{equation}
...
that is, the equivalent to Eqs.~\eqref{eq:A30intermsofR3} and \eqref{eq:A30withR30Sub} before
...
\subsubsection{Collinear Limit and the Altarelli--Parisi Splitting Function}\label{subsubsec:a30CollinearLimit}
...
\textit{AntCalc} in its current version. Each component is presented in turn below.
...
The quark-loop unintegrated antenna is computed using,
...
The unintegrated antennae are computed using,
...
term. Each component is presented in turn below.
...
and $\tilde A_4^0$. It represents the $\gamma^*\to qgg\bar q$ process. As with $B_4^0$
and $C_4^0$ below, the complete unintegrated expressions for all four routes are
lengthy and are collected in Appendix~\ref{ap:antExpressions}.
...
The result in terms of the antenna's master
integrals yields,
...
\textit{AntCalc} implements the complete massive $A_3^0$ build-and-integrate pipeline;
the route remains exploratory in the sense that, unlike the massless set, it has not
yet been checked against a global observable (see Table~\ref{tab:masterValidationStatus}).
To integrate it, two new MI are required. These two massive MI are,
\begin{equation}
    \begin{split}
...
are described as in Eq.~\eqref{eq:phaseSpaces} with $k_1^2=k_2^2=m_q^2$, $k_3^2=0$, and 
$q^2>4m_q^2$.
