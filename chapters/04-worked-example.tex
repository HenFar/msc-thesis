...
as discussed in Subsection~\ref{subsec:MIs}\footnote{The Passarino-Veltman MI are most commonly
...
common set of master integrals. Table~\ref{tab:MI} of Appendix~\ref{ap:masterIntegrals} summarises the
master integrals required for the antenna functions considered in this chapter. These master integrals
are defined and evaluated there, organised by antenna family; in this chapter, only $R_3$,
which is needed for the worked example of Subsection~\ref{sec:matDerivation}, and the massive master
integrals of Section~\ref{sec:massiveA30} are treated in the main text.
Evaluating the massless master integrals analytically was a crucial part of this work: they were all
derived explicitly, and not merely taken from a source, and their results reproduce the analytic
expressions known from the literature \cite{Gehrmann-DeRidder:2005btv,GehrmannDeRidder0311276}.
Appendix~\ref{ap:masterIntegrals} presents their definitions and results, and the derivation of
$R_{8,a}$, the most involved of them, is given step by step as an example of the method.
...
\iffalse
Both Passarino-Veltman and IBP MI are shown in the table, but they are quite different in structure
and integration processes and will be discussed separately. The Passarino-Veltman MI are $B_0$, $C_0$,
...
Appendix~\ref{subsec:PaVe}. The remainder of this chapter therefore focuses on the antenna functions and
on the master integrals arising from the IBP reduction. For each antenna set, the integral family and the master integrals
required for its integration are collected, by antenna family, in Appendix~\ref{ap:masterIntegrals},
where their definitions and analytic evaluation are given, including a detailed derivation of the
representative master integral $R_{8,a}$. Different colour components of the same antenna set
generally reduce to a common set of master integrals and are therefore discussed together.
...
$\gamma^*(p)\to q(k_1)g(k_3)\bar q(k_2)$. At this order, the amplitude receives contributions from the
two Feynman diagrams shown in Fig.~\ref{fig:feynDiagsA30}, corresponding to gluon emission from the
quark and antiquark lines. The complete tree-level amplitude is obtained by summing these two
contributions, and $A_3^0$ is constructed from the resulting squared matrix element after
normalisation to the corresponding two-parton Born contribution, as defined in
...
remaining coupling and colour factors are removed by the antenna normalisation. Equation~\eqref{eq:A30}
is therefore purely kinematic. In Chapter~\ref{ch:physicsbackground}, these factors were already
absorbed into the Born-normalised matrix-element notation; here they have been retained explicitly to
illustrate the normalisation.
...
\begin{equation}
    R_3 = \int d\Phi_3.
\end{equation}
Here, in $d$ dimensions, the massless two- and three-particle phase-space measures are
...
\begin{equation}
    ...
    \end{split}
\end{equation}
where the shorthands $d\mathcal S_n$ and $s_{123}$ and the solid-angle factors $\Omega_n$ are defined, together
with the four-particle measure, in Appendix~\ref{sec:MInotation}. With this parametrisation, the integral
can be written as \cite{TM_JoanaReis},
\begin{equation}
    \begin{split}
        R_3 &= \int d\Phi_3\\
...
Integrating antennae using \textit{AntCalc} is also simple. Every step of the analytic integration above
...
$\gamma^*\to q\bar q$, whose contributing Feynman diagram is shown in Fig.~\ref{fig:diagA21}.
...
$\gamma^*\to q\bar q$.
Representative two-loop Feynman diagrams contributing to this amplitude are shown in Fig.~\ref{fig:diagA22}.
...
\texttt{Out[2]:=}
    \begin{equation*}
4^{2\epsilon-3}\pi^{2\epsilon-4}e^{-2\gamma_E\epsilon}\left[\frac1{4\epsilon^4}+\frac{17}{8\epsilon^3}+\cdots\right]
    \end{equation*}
\end{mdframed}

For readability, only the leading terms of the bracket are shown. Since a two-parton antenna has no
unresolved phase space, the built antenna already contains the complete loop integration and differs
from the integrated antenna only by the normalisation of Eq.~\eqref{eq:antennaIntegral}. The prefactor
in \texttt{Out[2]} is exactly $1/C(\epsilon,2)=S_\epsilon^2/G_2$, and the bracket coincides, order by
order, with the integrated result \texttt{Out[5]} below. The output is real and free of $s_{12}$: the
timelike continuation has been applied and the renormalisation scale is set to $\mu^2=q^2$. For the
leading-colour and quark-loop components it already contains the coupling-renormalisation counterterm,
$-\tfrac{11}{6\epsilon}A_2^1$ and $+\tfrac{1}{3\epsilon}A_2^1$, respectively. The same form holds for
the other three components below.
...
are discussed in Appendix~\ref{sec:MIA22} and are evaluated at $q^2=1$, together with the one-loop functions $B_0^\dagger$ and
$C_0^\dagger$ of the ultraviolet counterterm (see Appendix~\ref{subsec:PaVe}):

\begin{mdframed}[backgroundcolor=gray!10, linewidth=0pt]
\texttt{In[3]:= A22Lead = BuildAndIntegrateAntenna[A,2,2,Component->Leading,\\ReturnMasterCombination->True];}\\
\texttt{In[4]:= (A22Lead/.A22MISubs)//Collect[\#,{A22LO,A3,A4,B0,C0},Simplify]\&}\\
\hfill
\texttt{Out[4]:=}
    \begin{equation*}
        \begin{gathered}
            -\frac{\left(-2\epsilon^2+\epsilon-2\right)^2}{8\pi^4\epsilon^2}A_{22,\text{LO}}
            +\frac{24\epsilon^6-40\epsilon^5+76\epsilon^4-88\epsilon^3+68\epsilon^2-33\epsilon+6}{4\pi^4\epsilon^3(2\epsilon-1)}A_3\\
            ...
            +\frac{11\cos(\pi\epsilon)}{6\epsilon}C_0^\dagger
            +\frac{11(3+2\epsilon)\cos(\pi\epsilon)}{12\epsilon}B_0^\dagger
        \end{gathered}
    \end{equation*}
\end{mdframed}

The combination in Out[4] is not yet the integrated antenna. Two further steps accompany the
substitution of the master integrals of Appendix~\ref{sec:MIA22}. First, these master integrals
are given in the $S_\Gamma$ normalisation of Eq.~\eqref{eq:SGamma} of Appendix~\ref{sec:MInotation}, whereas the integrated antennae are
...
timelike continuation is taken as $(-q^2)^{-2\epsilon}\to(q^2)^{-2\epsilon}\cos(2\pi\epsilon)$. The
counterterm terms, $-\tfrac{11}{6\epsilon}\mathcal A_2^1$ for the leading-colour component and
$+\tfrac{1}{3\epsilon}\mathcal A_2^1$ for the quark-loop component, are already part of Out[4]; their
timelike continuation, $\cos(\pi\epsilon)$, is explicit there, so the second step does not apply to
them. The two steps are common to the whole set, except that $\breve A_2^2$ involves no such timelike
phase factor.
...
\texttt{Out[2]:=}
    \begin{equation*}
4^{2\epsilon-3}\pi^{2\epsilon-4}e^{-2\gamma_E\epsilon}\left[-\frac1{4\epsilon^4}-\frac3{4\epsilon^3}+\cdots\right]
    \end{equation*}
\end{mdframed}
...
definitions and analytic expressions are presented in Appendix~\ref{sec:MIA22}.
...
\texttt{Out[4]:=}
    \begin{equation*}
        \begin{gathered}
            \frac{\left(-2\epsilon^2+\epsilon-2\right)^2}{8\pi^4\epsilon^2}A_{22,\text{LO}}
            +\frac{108\epsilon^7+780\epsilon^6-2641\epsilon^5+3305\epsilon^4-2476\epsilon^3+1210\epsilon^2-330\epsilon+36}{8\pi^4\epsilon^3\left(8\epsilon^2-6\epsilon+1\right)}A_3\\
            ...
            -\frac{\epsilon^3+4\epsilon^2-2\epsilon+2}{8\pi^4(4\epsilon-1)}A_6
        \end{gathered}
    \end{equation*}
\end{mdframed}
...
Representative examples of these diagrams are shown in Fig.~\ref{fig:diagA22}(d).
...
\texttt{Out[2]:=}
    \begin{equation*}
4^{2\epsilon-3}\pi^{2\epsilon-4}e^{-2\gamma_E\epsilon}\left[-\frac1{4\epsilon^3}-\frac1{9\epsilon^2}+\cdots\right]
    \end{equation*}
\end{mdframed}

After IBP reduction, the $N_f$ contribution reduces to the single master integral $A_4$, in contrast
to the leading- and subleading-colour components considered above, besides the counterterm functions
$B_0^\dagger$ and $C_0^\dagger$. The definition and analytic
evaluation of $A_4$ are presented in Appendix~\ref{sec:MIA22}.

\begin{mdframed}[backgroundcolor=gray!10, linewidth=0pt]
\texttt{In[3]:= A22Nf = BuildAndIntegrateAntenna[A,2,2,Component->Nf,\\ReturnMasterCombination->True];}\\
\texttt{In[4]:= (A22Nf/.A22MISubs)//Collect[\#,{A4,B0,C0},Simplify]\&}\\
\hfill
\texttt{Out[4]:=}
    \begin{equation*}
        \begin{gathered}
            -\frac{(\epsilon-1)\left(6\epsilon^3-5\epsilon^2+3\epsilon-2\right)}{2\pi^4\epsilon(2\epsilon-3)(3\epsilon-2)}A_4\\
            -\frac{\cos(\pi\epsilon)}{3\epsilon}C_0^\dagger
            -\frac{(3+2\epsilon)\cos(\pi\epsilon)}{6\epsilon}B_0^\dagger
        \end{gathered}
    \end{equation*}
\end{mdframed}
...
\texttt{Out[4]:=}
    ...
    \end{equation*}
\end{mdframed}
Out[4] is quoted before AntCalc's conversion factor between the raw and the public master normalisations,
which is exactly $-1/\cos(2\pi\epsilon)$ and is included when the master
integral is substituted.
...
The master integrals $A_{22,\text{LO}}$, $A_3$, $A_4$ and $A_6$ appearing in the results above are defined and evaluated in
Appendix~\ref{sec:MIA22}.
...
antenna discussed in Subsection~\ref{sec:matDerivation}.
...
\input{figures/diagrams-a31}

\subsection{$A_3^1$}\label{subsec:A31lead}

We begin with the leading-colour component $A_3^1$.
Representative one-loop Feynman diagrams entering its construction are shown in Fig.~\ref{fig:diagA31}.
...
\texttt{Out[2]:=}
    \begin{equation*}
        \frac{22s_{12}^6-22\epsilon s_{12}^6+66s_{12}^5s_{13}-66\epsilon s_{12}^5s_{13}+\cdots}{6(\epsilon-1)\epsilon\,q^2s_{13}s_{23}(s_{12}+s_{13})^2(s_{12}+s_{23})^2}
    \end{equation*}
\end{mdframed}
...
\texttt{In[4]:= (A31Lead/.A31MISubs)//Collect[\#,{R3,V5a,V5b,V8},Simplify]\&}\\
\hfill
\texttt{Out[4]:=}
    \begin{equation*}
        \begin{gathered}
            \frac{11\cdot4^{\epsilon-2}e^{-\gamma_E\epsilon}\left(2\epsilon^3-5\epsilon^2+6\epsilon-2\right)\pi^{\epsilon-4}}{3\epsilon^3}R_3\\
            -\frac{i\left(24\epsilon^6-40\epsilon^5+76\epsilon^4-88\epsilon^3+68\epsilon^2-33\epsilon+6\right)}{2\pi^2\epsilon^3(2\epsilon-1)}V_{5,a}\\
            +\frac{i\left(9\epsilon^5+25\epsilon^4-34\epsilon^3+41\epsilon^2-20\epsilon+4\right)}{2\pi^2\epsilon^3}V_{5,b}
        \end{gathered}
    \end{equation*}
\end{mdframed}

which expresses the integrated antenna in terms of the relevant master integrals. The first
term is the ultraviolet counterterm, $-\frac{11}{6\epsilon}$ times the master combination of the
tree-level antenna of Subsection~\ref{sec:matDerivation}, written through its master integral $R_3$.
The remaining terms are the one-loop contribution, expressed through $V_{5,a}$ and $V_{5,b}$; the
master integral $V_8$ does not appear in the leading-colour component. The definitions and analytic
expressions of these master integrals are presented in Appendix~\ref{sec:MIA31}. The masters are
evaluated at $q^2=1$, and the combination is returned without the common prefactor
$2^{10-6\epsilon}e^{2\gamma_E\epsilon}\pi^{7-3\epsilon}\Gamma(2-2\epsilon)/\Gamma(1-\epsilon)$, which
\textit{AntCalc} reports separately and which is shared by all three components of the set.
...
\texttt{Out[2]:=}
    \begin{equation*}
        -\frac{4s_{12}^6}{(\epsilon-1)q^2(s_{12}+s_{13})^2(s_{12}+s_{23})^2(s_{13}+s_{23})^2}B_0\!\left(q^2;0,0\right)+\cdots
    \end{equation*}
\end{mdframed}
...
\texttt{Out[4]:=}
    \begin{equation*}
        \begin{gathered}
            \frac{i\left(\epsilon^3+4\epsilon^2-2\epsilon+2\right)}{2\pi^2(4\epsilon-1)}V_8\\
            -\frac{i\left(108\epsilon^7+780\epsilon^6-2641\epsilon^5+3305\epsilon^4-2476\epsilon^3+1210\epsilon^2-330\epsilon+36\right)}{4\pi^2\epsilon^3\left(8\epsilon^2-6\epsilon+1\right)}V_{5,a}\\
            +\frac{i\left(49\epsilon^5+125\epsilon^4-242\epsilon^3+185\epsilon^2-94\epsilon+20\right)}{2\pi^2\epsilon^3}V_{5,b}
        \end{gathered}
    \end{equation*}
\end{mdframed}
...
This result is noteworthy, as from the leading- to the subleading-colour component of this $A_3^1$ antenna, 
...
The $N_f$-dependent component $\hat A_3^1$ has a different origin from the
...
$\hat A_3^1$ antenna therefore arises entirely from the $N_f$-dependent part of
...
yielding the corresponding $\hat A_3^1$ antenna. Since the unrenormalised
...
of $\hat A_3^1$ follows directly from the tree-level calculation presented in
Subsection~\ref{sec:matDerivation}. In particular, its phase-space integration inherits
...
$\hat{\mathcal A}_3^1$, with the result above beginning at $1/\epsilon^3$.

The master integrals $V_{5,a}$, $V_{5,b}$ and $V_8$ appearing in the results above are defined and evaluated in
Appendix~\ref{sec:MIA31}.
...
expressed in terms of the four-particle phase-space master integrals of
Appendix~\ref{sec:MIfourParton}. Since the complete unintegrated expressions are lengthy, only the
features relevant to their construction and integration are presented here; the full
expressions are collected in Appendix~\ref{ap:antExpressions}, and their local
infrared validation is given in Appendix~\ref{sec:localValidationExtra}.

\input{figures/diagrams-a40}

\input{figures/diagrams-bc40}
...
\end{equation*}
Representative tree-level Feynman diagrams contributing to this process are shown in Fig.~\ref{fig:diagA40}.
...
definitions and analytic evaluation are discussed in Appendix~\ref{sec:MIfourParton}.
...
Subsection~\ref{subsec:A40}, but corresponds to the subleading term in the colour decomposition of
...
$R_{8,b}$, discussed in Appendix~\ref{sec:MIfourParton}. Substituting their analytic
...
Representative tree-level Feynman diagrams contributing to this process are shown in Fig.~\ref{fig:diagBC40}.
...
The master integrals $R_4$, $R_6$, $R_{8,a}$ and $R_{8,b}$ appearing in the results above are defined and evaluated in
Appendix~\ref{sec:MIfourParton}.
...
integration; the general consequences of this extension for the antenna subtraction and
reduction framework are discussed in Appendix~\ref{sec:massiveExtension}.
...
\textit{LiteRed2} and the phase-space normalisation of the literature masters. This factor multiplies both
of the relations below and reflects the different normalisation conventions of the two sets of masters
rather than any physical effect; it was found by comparing $J_1^{(m_q,0,m_q)}$ with
$I_1^{(m_q,0,m_q)}$.
The resulting transformation is,
\begin{equation}\label{eq:massiveMasterMap}
    \begin{split}
...
in the massless case discussed in Subsection~\ref{sec:matDerivation}, with the quark mass specified
...
The massive IBP reduction described above was carried out with \textit{LiteRed2}, and its result, converted
to the literature masters through Eq.~\eqref{eq:massiveMasterMap}, reproduces the analytic expression of
Ref.~\cite{GehrmannDeRidder:2009fz}. In the default configuration, \textit{AntCalc} returns this result
directly as a stored closed form, evaluated and expanded in $\epsilon$.
...
This chapter has presented the construction and analytic integration of the massless quark--antiquark
antenna functions implemented in \textit{AntCalc} through NNLO, together with an exploratory extension
to the massive $A_3^0$ antenna. The purpose of this final section is to summarise the resulting antenna
set, the integration methods required for each contribution, and the checks used to validate the
results.

The implemented routes span the different perturbative contributions discussed throughout this
chapter: the NLO tree-level and virtual antennae $A_3^0$ and $A_2^1$; the NNLO double-virtual $A_2^2$
set; the real--virtual $A_3^1$ set; and the double-real four-parton antennae $A_4^0$, $\tilde A_4^0$,
$B_4^0$, and $C_4^0$. The exploratory massive $A_3^0$ route discussed in
Section~\ref{sec:massiveA30} is included separately.

Table~\ref{tab:masterValidation} summarises, for each antenna, the construction and integration method
together with the corresponding validation evidence. It provides a compact overview of the
calculations presented in this chapter and distinguishes the fully validated massless antenna set from
the exploratory massive extension.

{\footnotesize
\renewcommand{\arraystretch}{1.0}
\begin{center}
\begin{longtable}{|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{4.0cm}|>{\raggedright\arraybackslash}p{1.9cm}|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{1.9cm}|}
\caption{Construction, integration and validation overview of every antenna route implemented in \textit{AntCalc}. The API call of each route is \texttt{BuildAndIntegrateAntenna[type, multiplicity, loop order]}; only the arguments are shown. ``Not applicable'' marks routes with no real unresolved parton to take a soft/collinear limit of ($A_2^0$, $A_2^1$, $A_2^2$); ``Not performed'' marks the one remaining route, the virtual $A_3^1$ set, whose real-radiation limits would require one-loop soft-current and splitting-amplitude factorisation rather than the tree-level method used throughout this table --- its infrared structure is instead validated via the pole cancellation shown in Chapter~\ref{ch:validation}. The massive $A_3^0$ route is deliberately marked as less mature than the massless set: it agrees with the literature locally, but has not yet been checked against any global observable.}\label{tab:masterValidation}\label{tab:masterValidationConstruction}\label{tab:masterValidationStatus}\\
\hline
\textbf{Route} & \textbf{Construction and integration} & \textbf{Literature (order)} & \textbf{Validation} & \textbf{Status} \\
\hline
\endfirsthead
\multicolumn{5}{l}{\textit{Table~\ref{tab:masterValidation} continued.}}\\
\hline
\textbf{Route} & \textbf{Construction and integration} & \textbf{Literature (order)} & \textbf{Validation} & \textbf{Status} \\
\hline
\endhead
\hline
\endlastfoot
$A_2^0$: $\gamma^*\to q\bar q$, LO; \texttt{[A,2,0]} & Self-interference, Born-normalised reference. Trivial (reference, $A_2^0\equiv1$). & --- (reference normalisation) & Local: not applicable. Global: LO normalisation of the $R$-ratio. & By construction \\ \hline
$A_3^0$: $\gamma^*\to qg\bar q$, NLO real; \texttt{[A,3,0]} & Self-interference; colour-coefficient extraction; matched to by-hand derivation. IBP / \textit{LiteRed2}, \texttt{X30} basis; MI: $R_3$. & \cite{Gehrmann-DeRidder:2005btv} (NLO) & Local: eikonal factor and AP splitting function reproduced. Global: feeds $r_\text{NLO}$. & Pass \\ \hline
$A_2^1$: $\gamma^*\to q\bar q$, NLO virtual; \texttt{[A,2,1]} & Tree--loop interference; Passarino-Veltman-reduced scalar extraction. Passarino-Veltman / \textit{FeynHelpers}; MI: $B_0$, $C_0$. & \cite{Gehrmann-DeRidder:2005btv} (NLO) & Local: not applicable. Global: feeds $r_\text{NLO}$. & Pass \\ \hline
$A_3^1$ set ($A_3^1$, $\tilde A_3^1$, $\hat A_3^1$): $\gamma^*\to qg\bar q$, NNLO, one loop; \texttt{[A,3,1]} & Tree--loop interference; colour-coefficient extraction (leading/subleading/quark-loop). IBP / \textit{LiteRed2}, \texttt{A31} basis; build-side MI are Passarino-Veltman scalar functions ($B_0,C_0,D_0$, Table~\ref{tab:MI}); integration MI: $V_{5,a}$, $V_{5,b}$, $V_8$. & \cite{Gehrmann-DeRidder:2005btv,Gehrmann-DeRidder:2004ttg} (NNLO) & Local: not performed. Global: feeds $r_\text{NNLO}$. & Pass \\ \hline
$A_2^2$ set ($A_2^2$, $\tilde A_2^2$, $\hat A_2^2$, $\breve A_2^2$): $\gamma^*\to q\bar q$, NNLO, two loops; \texttt{[A,2,2]} & Two-loop tree interference and one-loop self-interference (two source contributions). IBP / \textit{LiteRed2}, \texttt{A22TwoLoopTree}/\allowbreak\texttt{A22OneLoopSelf} bases; MI: $A_{22,\text{LO}}$, $A_3$, $A_4$ (also $A_6$ for $\tilde A_2^2$). & \cite{Gehrmann-DeRidder:2004ttg} (NNLO; $A_2^2,\tilde A_2^2,\hat A_2^2$), \cite{Jakubcik:2022zdi} (NNLO; $\breve A_2^2$) & Local: not applicable. Global: feeds $r_\text{NNLO}$. & Pass \\ \hline
$A_4^0$, $\tilde A_4^0$: $\gamma^*\to qgg\bar q$, NNLO, double-real; \texttt{[A,4,0]} & Colour-ordered amplitude; colour-coefficient extraction. IBP / \textit{LiteRed2}, \texttt{X40} basis; MI: $R_4$, $R_6$ ($A_4^0$); $R_4$, $R_6$, $R_{8,a}$, $R_{8,b}$ ($\tilde A_4^0$). & \cite{Gehrmann-DeRidder:2005btv} (NNLO) & Local: both colour components validated: eikonal and AP/spin-correlated splitting functions for the leading term; dipole eikonal and absent $gg$ collinear singularity for the subleading term (Appendix~\ref{sec:localValidationExtra}). Global: feeds $r_\text{NNLO}$. & Pass \\ \hline
$B_4^0$: $\gamma^*\to q\bar q q^\prime\bar q^\prime$, NNLO; \texttt{[B,4,0]} & Sector self-interference (direct amplitude only). IBP / \textit{LiteRed2}, \texttt{X40} basis; MI: $R_4$, $R_6$. & \cite{Gehrmann-DeRidder:2005btv} (NNLO) & Local: single-collinear ($q^\prime\bar q^\prime\to g$), double-soft, and triple-collinear limits reproduced (Appendix~\ref{subsec:localValidationB40}). Global: feeds $r_\text{NNLO}$. & Pass \\ \hline
$C_4^0$: $\gamma^*\to q\bar q q\bar q$, NNLO; \texttt{[C,4,0]} & Sector symmetrised interference (direct and exchange). IBP / \textit{LiteRed2}, \texttt{X40} basis; MI: $R_4$, $R_6$, $R_{8,b}$. & \cite{Gehrmann-DeRidder:2005btv} (NNLO) & Local: single-collinear region confirmed regular; identical-flavour triple-collinear splitting function reproduced (Appendix~\ref{subsec:localValidationC40}). Global: feeds $r_\text{NNLO}$. & Pass \\ \hline
Massive $A_3^0$: $\gamma^*\to q(m_q)g\bar q(m_q)$, exploratory; \texttt{[A,3,0,\allowbreak quarkMass->\allowbreak mQ]} & Same route as massless $A_3^0$, with the \texttt{quarkMass} option. IBP / \textit{LiteRed2}, massive family (default call returns the resulting stored closed form); MI mapped from \textit{LiteRed2}'s $J_1$, $J_2$ to the literature $I_1^{(m_q,0,m_q)}$, $I_2^{(m_q,0,m_q)}$. & \cite{GehrmannDeRidder:2009fz} & Local: soft-gluon eikonal and quasi-collinear splitting function reproduced (Appendix~\ref{subsec:localValidationMassiveA30}). Global: not applicable (does not feed the massless $R$-ratio). & Literature agreement, incl. local soft/quasi-collinear limits; no global observable validation yet \\ \hline
\end{longtable}
\end{center}
}
