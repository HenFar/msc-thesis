...
The methods \textit{AntCalc} uses to build and then integrate the antenna functions
described in Chapter~\ref{ch:physicsbackground} were discussed in detail in
Chapter~\ref{ch:packageframework}. In this chapter we will construct all antenna
functions that \textit{AntCalc} is currently able to handle, by first building their
unintegrated form, then their master combination form, that is before the master
integral substitutions, and finally the $\epsilon$-series integrated antenna form.

The chapter will begin with a thorough example of the complete building and
integration of an antenna, the $A_3^0$ antenna --- first by hand, and then using
\textit{AntCalc}, comparing the results at relevant steps and ensuring their equality.
Finally, the unintegrated antenna is locally validated by computing its soft and
collinear limits to yield the eikonal factor and the Altarelli--Parisi splitting
function. This local validation method can also be used with other antenna functions
whenever the corresponding unresolved limits are present. This first section is
intended to showcase the inner algebraic steps \textit{AntCalc} pursues internally.
After this longer section, several smaller sections follow, each giving a much
abbreviated walkthrough of the building, master combination, and integration of the
remaining implemented antenna functions. At the very end, the one massive antenna,
$A_3^0$, is also explored in some detail, and the differences between its route and
that of its massless analogue are compared.

The massless antenna results shown in the present chapter were validated against the
literature and, in Chapter~\ref{ch:validation}, were globally validated by computing
the hadronic massless $R$-ratio up to NNLO. A complete look through all antennae
considered and used in the $R$-ratio, plus the massive extension, alongside their
respective processes, and other relevant information is provided at the very end of
this chapter in Section~\ref{sec:masterValidationOverview}.

\textit{AntCalc} makes use of different master integrals for each antenna, but it is
noteworthy that components of the same antenna set share the same overall pool of
master integrals. Table~\ref{tab:MI}, below, shows the build-side and integration-side
master integrals used by each antenna function, and it is visible that sets like
$A_2^2$ or $A_3^1$ share the same overall pool of masters.
...
As described in Chapter~\ref{ch:packageframework}, \textit{AntCalc} makes use of two
distinct integral reduction methods: IBP and PaVe. The second one is well established
and used only for reducing unintegrated one- and two-loop antennae to their canonical
form. As such, its method and its master integrals are discussed only in
Appendix~\ref{subsec:PaVe}. In this chapter, after each family is introduced and
discussed in its own section, a final subsection discussing the master integrals used
within it is present. This is due to the way in which the package builds the complete
set at once, and only separates them at the end, before integrating. A lengthier
step-by-step derivation of one of these masters, $R_{8,a}$, in this case, is present in
Subsubsection~\ref{subsubsec:R8a}. These IBP MI are computed over different
phase-spaces, depending on the number of 
...
\begin{equation}\label{eq:phaseSpaces}
    \begin{split}
        ...
        &d\Phi_3 = (2\pi)^{3-2d}2^{-1-d}\!\left(q^2\right)^{2-\frac d2}(s_{12}s_{13}s_{23})^{\frac{d-4}{2}}d\Omega_{d-1}d\Omega_{d-2}d\mathcal S_3\delta\!\left(q^2-s_{123}\right)\\
        &\begin{split}
    ...
    \end{split}
\end{equation}
Here $\Delta_4$ is the Gram determinant given by the Källén function $\lambda$
\cite{GehrmannDeRidder0311276,Anastasiou:2002yz},
\begin{equation}
...
To provide a general result and prevent notation conflicts, \textit{AntCalc} returns
its master integral combinations as expressions including $j[...]$ functions, the
default notation of \textit{LiteRed2}. In this chapter we shall, at the master
combination stage for each antenna, include a substitution of those functions for
their general notation forms ($A_3$, $A_4$, $R_3$, etc.). The substitution arrays used
are shown in full in Appendix~\ref{sec:MISubsArrays}.
...
diagrams are shown below in Fig.~\ref{fig:feynDiagsA30}.
...
\begin{equation}
    \begin{split}
        R_3 &= \int d\Phi_3\\
            ...
            &= \frac1{128q^2\pi^3}\int d\mathcal S_3\,\delta\!\left(q^2-s_{123}\right).
    \end{split}
\end{equation}
...
Integrating antennae using \textit{AntCalc} is also simple. Every step in Subsubsection~\ref{sec:antIntMaths}
...
\section{Two-Parton Contributions: the $A_2^2$ Set}\label{subsec:A22}

\subsection{The $A_2^2$ Antenna}
...
\textit{AntCalc} in its current version. The leading unintegrated antenna is computed using,
...
\texttt{Out[2]:=}
    \begin{equation*}
        \frac{11\Big[(3+2\epsilon)B_0\!\left(s_{12};0,0\right)+2s_{12}C_0\!\left(0,0,s_{12};0,0,0\right)\Big]}{12\epsilon}+...
    \end{equation*}
\end{mdframed}

The result in terms of the antenna's master integrals yields,
...
Finally, the epsilon-series result is computed as,
...
The integrated Laurent series agrees with the established $\mathcal A_2^2$ result
\cite{Gehrmann-DeRidder:2004ttg}.

\subsection{The $\tilde A_2^2$ Antenna}

The $A_2^2$ route is computed similarly to $A_2^1$. This route, however, yields four
different antenna components and is the longest-running calculation supported by
\textit{AntCalc} in its current version. The subleading unintegrated antenna is computed using,
...
The integrated Laurent series agrees with the established $\tilde{\mathcal A}_2^2$ result
\cite{Gehrmann-DeRidder:2004ttg}.

\subsection{The $\hat A_2^2$ Antenna}

The $A_2^2$ route is computed similarly to $A_2^1$. This route, however, yields four
different antenna components and is the longest-running calculation supported by
\textit{AntCalc} in its current version. The quark-loop unintegrated antenna is computed using,
...
\texttt{Out[4]:=}
    \begin{equation*}
        -\frac{(\epsilon-1)\left(6\epsilon^3-5\epsilon^2+3\epsilon-2\right)}{\pi^4\epsilon(2\epsilon-3)(3\epsilon-2)}A_4
    \end{equation*}
\end{mdframed}
...
The integrated Laurent series agrees with the established $\hat{\mathcal A}_2^2$ result
\cite{Gehrmann-DeRidder:2004ttg}.

\subsection{The $\breve A_2^2$ Antenna}
...
\texttt{Out[2]:=}
    \begin{equation*}
        \frac{\Gamma^2(1+\epsilon)\Gamma^6(1-\epsilon)}{4\epsilon^4\Gamma^2(2-2\epsilon)}+...
    \end{equation*}
\end{mdframed}
...
\cite{Gehrmann-DeRidder:2004ttg} apart from the exception discussed in
Subsubsection~\ref{subsubsec:breveA22}, below.

\subsubsection{A Note on Sources}\label{subsubsec:breveA22}

During the construction of the $\breve A_2^2$ antenna the exact formulation of the $\epsilon^{-1}$ 
antenna infrared pole failed to match the expression in Eq.~(4.10) of \cite{Gehrmann-DeRidder:2004ttg}, 
our main source. Using the paper's expression also resulted in a failed cancellation on the massless 
hadronic $R$-ratio's $\epsilon^{-1}$ infrared pole on the NNLO term. Tracing this discrepancy through 
the assembly of colour channels revealed an uncancelled $28C_F^2\zeta_3/(3\epsilon)$ residue, which pointed 
at a sign error in the $7\zeta_3/(6\epsilon)$ term of the antenna.

This discrepancy with the main source was independently confirmed in the literature by Jakubčík, Marcoli, 
and Stagnitto~\cite{Jakubcik:2022zdi} (Eq.~B.7) formally identified and published the typographical 
error in Eq.~(4.10) of \cite{Gehrmann-DeRidder:2004ttg}. In \textit{AntCalc}, 
the $\breve A_2^2$ antenna is integrated directly from the one-loop self-interference 
matrix element, natively reproducing the corrected Laurent series of \cite{Jakubcik:2022zdi} 
and ensuring exact pole cancellation in the NNLO $R$-ratio term.


\subsection{Master Integrals}\label{subsec:A22mi}
...
\section{Three-Parton Contributions: the $A_3^1$ Set}

\subsection{The $A_3^1$ Antenna}
...
term. The leading unintegrated antenna is computed using,
...
The integrated Laurent series agrees with the established $\mathcal A_3^1$ result
\cite{Gehrmann-DeRidder:2005btv,Gehrmann-DeRidder:2004ttg}.

\subsection{The $\tilde A_3^1$ Antenna}


Computing the $A_3^1$ antenna set is very similar to the work done for the $A_2^2$ set,
with the big differentiator being that this set does not include a self-interference
term. The subleading unintegrated antenna is computed using,
...
The integrated Laurent series agrees with the established $\tilde{\mathcal A}_3^1$ result
\cite{Gehrmann-DeRidder:2005btv,Gehrmann-DeRidder:2004ttg}.

\subsection{The $\hat A_3^1$ Antenna}


Computing the $A_3^1$ antenna set is very similar to the work done for the $A_2^2$ set,
with the big differentiator being that this set does not include a self-interference
term. The quark-loop unintegrated antenna is computed using,
...
Unlike $A_3^1$ and $\tilde A_3^1$, the result in terms of the antenna's master
integrals vanishes for $\hat A_3^1$,

\begin{mdframed}[backgroundcolor=gray!10, linewidth=0pt]
\texttt{In[3]:= A31Nf = BuildAndIntegrateAntenna[A,3,1,Component->Nf,\\ReturnMasterCombination->True];}\\
\texttt{In[4]:= (A31Nf/.A31MISubs)//Collect[\#,{V5a,V5b,V8},Simplify]\&}\\
\hfill
\texttt{Out[4]:=}
    \begin{equation*}
        0
...
since this route has no bare one-loop IBP contribution left to reduce. $\hat A_3^1$'s
non-zero content instead enters afterwards, as a UV coupling-renormalisation
counterterm built from the already-integrated tree-level result of
Section~\ref{sec:matDerivation}; its leading piece,
\begin{equation}
    \frac1{3\epsilon}\mathcal A_3^0,
\end{equation}
already reproduces the $\epsilon^{-3}$ and $\epsilon^{-2}$ poles of the integrated
result below exactly. Finally, the epsilon-series result is computed as,
...
The integrated Laurent series agrees with the established $\hat{\mathcal A}_3^1$ result
\cite{Gehrmann-DeRidder:2005btv,Gehrmann-DeRidder:2004ttg}.
...
\section{Four-Parton Contributions}

\subsection{The $A_4^0$ Antenna}
...
The leading unintegrated antenna is computed using,
...
The complete unintegrated expression is collected in
Appendix~\ref{ap:antExpressions}. The result in terms of the antenna's master
integrals yields,

\begin{mdframed}[backgroundcolor=gray!10, linewidth=0pt]
\texttt{In[3]:= A40Lead = BuildAndIntegrateAntenna[A,4,0,Component->Leading,\\ReturnMasterCombination->True];}\\
\texttt{In[4]:= (A40Lead/.A40MISubs)//Collect[\#,{R4,R6,R8a,R8b},Simplify]\&}\\
\hfill
\texttt{Out[4]:=}
    \begin{equation*}
        \begin{gathered}
...
The integrated result agrees with
the established $\mathcal A_4^0$ result \cite{Gehrmann-DeRidder:2005btv}.

\subsection{The $\tilde A_4^0$ Antenna}


The double real-radiation antenna set is $A_4^0$, and it has two components, $A_4^0$ 
and $\tilde A_4^0$. It represents the $\gamma^*\to qgg\bar q$ process.

The subleading unintegrated antenna is computed using,
...
\texttt{In[4]:= (A40Sub/.A40MISubs)//Collect[\#,{R4,R6,R8a,R8b},Simplify]\&}\\
\hfill
\texttt{Out[4]:=}
    \begin{equation*}
        \begin{gathered}
...
The integrated result agrees with
the established $\tilde{\mathcal A}_4^0$ result \cite{Gehrmann-DeRidder:2005btv}.

\subsection{The $B_4^0$ Antenna}
...
The $B_4^0$ antenna building process is completed as usual using \textit{AntCalc},
...
\texttt{In[4]:= (B40/.A40MISubs)//Collect[\#,{R4,R6,R8a,R8b},Simplify]\&}\\
\hfill
\texttt{Out[4]:=}
    \begin{equation*}
        \begin{gathered}
...
The integrated result agrees with
the established $\mathcal B_4^0$ result \cite{Gehrmann-DeRidder:2005btv}.

\subsection{The $C_4^0$ Antenna}
...
The $C_4^0$ antenna building process is completed as usual using \textit{AntCalc},
...
\texttt{In[4]:= (C40/.A40MISubs)//Collect[\#,{R4,R6,R8a,R8b},Simplify]\&}\\
\hfill
\texttt{Out[4]:=}
    \begin{equation*}
        \begin{gathered}
...
The integrated result agrees with
the established $\mathcal C_4^0$ result \cite{Gehrmann-DeRidder:2005btv}.
...
commands as for the massless $A_3^0$ route, as discussed in Subsubsection~\ref{subsec:masslessA30},
...
\endhead
\hline
\caption{Validation evidence and status of every antenna route implemented in \textit{AntCalc}. ``Not applicable'' marks routes with no real unresolved parton to take a soft/collinear limit of ($A_2^0$, $A_2^1$, $A_2^2$); ``Not performed'' marks routes where such a limit exists but was not checked in this work --- see the chapter introduction for why $A_3^0$ was chosen to demonstrate the method rather than repeating it on every route. The massive $A_3^0$ route is deliberately marked as less mature than the massless set: it agrees with the literature locally, but has not yet been checked against any global observable.}
