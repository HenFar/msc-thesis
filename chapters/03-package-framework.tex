...
\textit{AntCalc}, version 0.3.2$\beta$4, corresponding to commit \texttt{6deae58}.
...
IBP reduction. Both are external dependencies that must be installed by the user prior to
loading \textit{AntCalc}; the package's own installation procedure only installs the
\textit{AntCalc} paclet itself.
...
antenna set associated with the process-level contribution $|M_{qg\bar q}^1|^2$ is $\{\text A,3,1\}$. This key
...
For loop routes, this public-output boundary is also where ultraviolet renormalisation is
applied. Renormalisation is not a discrete internal stage of the build workflow; instead, it is
applied to the unintegrated one- and two-loop antennae precisely at the point where they cross
from the internal (backend) representation into the public antenna expression returned to the
user, using the coupling rescaling of Eq.~\eqref{eq:alphaRescaling}. This is why no separate
renormalisation step appears explicitly in Fig.~\ref{fig:antcalc-architecture-buildstage}: it
coincides with the ``Route \& Public Boundary'' crossing already shown there.
...
has been described previously in Eq.~\eqref{eq:colourNorm}, and the 
...
involved inner structure of the $|M_{4q}^0|^2$ process-level contribution, requiring a correspondingly more complex
extraction process.
...
interference. The interference is colour- and one-loop-normalised, as described in Eqs.~\eqref{eq:loopNorm} and \eqref{eq:colourNorm}.
...
Loop-order-two routes are dispatched to \texttt{BuildTwoLoopAntennaData}. Within the massless
$q\bar q$ final-final antennae considered up to NNLO, the only two-loop antenna set is $A_2^2$.
Unlike the one-loop routes, the two-loop source is not reduced with Passarino-Veltman reduction.
On the build side, the two loop momenta $\ell_1$ and $\ell_2$ are removed directly by IBP reduction
with \textit{LiteRed2}, and the known analytic expressions of the resulting master integrals are
substituted, so that the public build result is real and expressed in terms of $\epsilon$ alone,
with the scale set to $\mu^2=q^2$. Passarino-Veltman reduction enters this route only through the unintegrated
$A_2^1$ antenna, which is attached to the leading-colour and quark-loop components as their
ultraviolet counterterm at the public build boundary described in Section~\ref{sec:buildStage}.
The four-component set is also not generated like the $A_4^0$ and $A_3^1$ sets were; rather, its
four components come from two distinct source contributions, as discussed in
...
as in Eq.~\eqref{eq:antennaIntegral}. The integrated antennae are denoted with calligraphic
letters $\mathcal X_n^l$.
...
Unlike the build stage, whose intermediate data are collected in a single route context, the
integration stage does not hold its route state in one context object; it is spread across
several objects, accessed as required: the key and profile, the input
...
As for the build stage, ultraviolet renormalisation at the integration stage is applied
independently, at the analogous public-output boundary: once the integrated one- and two-loop
antennae are extracted, immediately before they are returned by \texttt{IntegrateAntenna[...]}.
This mirrors, rather than reuses, the renormalisation applied during the build stage, since the
two calls can be made independently of one another. No separate renormalisation step therefore
appears in Fig.~\ref{fig:antcalc-architecture-intstage} either: it coincides with the ``Public
Integration Views'' crossing shown there.
...
\renewcommand{\arraystretch}{1.3}
  \begin{tabular}{|c|c|c|} 
   \hline
   \textbf{Default Backend} & \textbf{Basis-Family Set} & \textbf{Current Route} \\
   \hline
...
The master integrals used in these reductions are defined and evaluated in
Appendix~\ref{ap:masterIntegrals}, while their role in the analytic integration of antenna
functions is illustrated through representative examples in Chapter~\ref{ch:workedexample}.
...
expanded as a Laurent series in $\epsilon$, yielding the integrated antenna function. The master
integrals and their analytic evaluation are presented in Appendix~\ref{ap:masterIntegrals}, and
representative applications of this procedure to the integration of antenna functions in
Chapter~\ref{ch:workedexample}. The complete IBP-based integration workflow is summarised
...
normalised using the $\mathcal N(\epsilon, k)$ factor, introduced in Eq.~\eqref{eq:normN}. 
...
\texttt{AntennaObject}. Equation~\eqref{eq:expansionOrderDiag} illustrates the use of
...
integration is given, much like it was for the build stage in \eqref{eq:a31Norm}, as,
...
Eq.~\eqref{eq:a31WithTs} and returning an analogous ordered list made up of the $\mathcal T$-objects,
as,
\begin{equation}
    \begin{gathered}
...
\begin{itemize}
    ...
    combinations used in the $R$-ratio, defined in Section~\ref{subsec:Tobjects} of
...
\texttt{BuildRRatio[SMQCD, maxOrder -> \ldots]}\footnote{The label \texttt{SMQCD} is not
standard in the literature. It is used here to denote the massless QCD sector of the Standard
Model.}, it generates the $R$-ratio through NNLO and, by default, returns the finite
coefficient after application of the observable convention bridge. This result has been validated against a
...
\label{tab:TObject}
\end{table}
As an example, let us consider $\mathcal T_{q\bar qg}^6$. As shown in Eq.~\eqref{eq:a31Norm}
...
It is currently applied to every massless public route with an external gluon leg, that is, to $A_3^0$,
...
OS X ARM (64-bit), and AntCalc 0.3.1$\beta$3.
