...
\texttt{AntennaProfile}), the amplitude generated from it, the sectors, computed interferences, 
components after separation, diagnostics, and its normalised interference. Depending on the 
route, the context might also retain prototype components, full colour components, reference 
square components or loop-specific amplitudes. 
...
they do it changes with the antenna's multiplicity. The different production methods are shown in completeness 
in Subsection~\ref{subsec:antFamilies}, when each of the elements of the \textit{Current Route} column is
discussed. 
The extraction paths are also implicitly discussed in that subsection. $A_2^0$ is set to $1$ as discussed. 
Both $A_3^0$ and the $A_4^0$-set antennae are directly extracted from the usual structure of 
$NX_n^l + 1/N\tilde X_n^l$. Lastly we extract the $B_4^0$ and $C_4^0$ antennae from the more complex 
inner structure of the $M_{4q}^0$ full-process amplitude and hence require a more complex extraction process.
