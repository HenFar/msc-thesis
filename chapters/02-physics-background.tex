...
bundling two processes with the same final-state particles in a single set.
...
\begin{equation}
    \begin{gathered}
        ...
        X_{ijk}^1 = S_{ijk,IK}\frac{|\mathcal M_{ijk}^1|^2}{|\mathcal M_{IK}^0|^2} - X_{ijk}^0\frac{\mathcal M_{IK}^1}{|\mathcal M_{IK}^0|^2}.
    \end{gathered}
\end{equation}
...
element loop-configurations for a given colour structure\footnote{Equivalently, this 
corresponds to stripping a factor of $2C_F g_s^{2(n-2+l)}$.}: for one, two, and three loops, given
as,
\begin{equation}
    \begin{gathered}
...
setting the Born-level antenna to one. The symmetry factors are also all equal 
...
\begin{itemize}\label{list:A31Comps}
    ...
    by the tilde antenna, e.g. $\tilde A_3^1$. An NNLO leading antenna-set 
