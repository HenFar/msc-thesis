...
bare coupling $\alpha_0$ from the QCD Lagrangian, as shown in Eq.~\ref{eq:qcdLagrangian},
are not finite beyond tree level, since their higher-order coefficients diverge
...
The use of CDR, and in particular the complete continuation to $d$-dimensional
space, introduces some complications, however. The move to $d$-dimensional
spherical coordinates means that the phase-space integrals are now performed over
$d$-dimensional solid angles, which produce characteristic factors such as powers
of $4\pi$, Euler's constant $\gamma_E$, and Gamma functions like $\Gamma(1-\epsilon)$.
These factors are a consequence of performing the angular and radial integrations
away from four dimensions, and depend on the conventions adopted for dimensional
regularisation. To express the integrated antennae in the $\overline{\text{MS}}$
(modified minimal subtraction)-compatible convention~\cite{Bardeen:1978yd} used
throughout this work, we multiply the raw $d$-dimensional integrals by the
normalisation factor $S_\epsilon=(4\pi)^{-\epsilon}e^{\epsilon\gamma_E}$. This
convention removes the ubiquitous $4\pi$ and $\gamma_E$ factors from the coefficients
of the perturbative expansion.

A separate normalisation arises from the convention used to express the
perturbative expansion in the strong coupling. In the standard QCD convention
adopted here, the expansion is written in terms of $\alpha_s/(2\pi)$, instead of
just $\alpha_s$, since the squared matrix elements scale with the whole raw
coupling constant $g^2=4\pi\alpha_s=8\pi^2(\alpha_s/(2\pi))$ rather than with
$\alpha_s$ alone. This makes a further normalisation parameter necessary. The scale
normalisation,
...
case. $\beta_0$ is the one-loop coefficient of the QCD beta
function~\cite{Ellis:1996mzs,Peskin:1995ev}, that tracks
...
\begin{equation*}
    \begin{split}
        ...
        &\hspace{4.8cm} +N_f\left(\mathcal B_4^0+\hat{\mathcal A_3^1}+\hat{\mathcal A_2^2}\right)+\left(N-\frac1N\right)\breve{\mathcal A_2^2}\Bigg]
    \end{split}
\end{equation*}
\begin{equation}
