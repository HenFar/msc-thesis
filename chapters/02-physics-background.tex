...
coupling constant $g_s^2=4\pi\alpha_s=8\pi^2(\alpha_s/(2\pi))$ rather than with
...
makes the global normalisation factor $\mathcal N(\epsilon,k)$, used throughout the present work,
\begin{equation}\label{eq:normN}
    \mathcal N(\epsilon,k) = \frac{C(\epsilon, k)}{\Phi_2}.
\end{equation}
The form given above is representative of this convention rather than an exact,
route-by-route prescription: the precise multiplicative bridging applied to a given
antenna also depends on the specific master-integral basis its integration reduces
to, and is validated numerically against the literature result for each public
route (Chapter~\ref{ch:workedexample}), rather than derived symbolically here.
...
respectively, through all topologies of the same process. Loop antennae also require us to denote the number
