...
$\gamma^*\to q(k_1)g(k_3)\bar q(k_2)$ process. If one of the quarks and the gluon 
become collinear, say $q(k_1)$ and $g(k_3)$, then $s_{13}\to 0$ and the amplitude 
has a single pole, $|\mathcal M|^2\sim1/s_{13}\to\infty$. If the gluon is soft,
then both $s_{13}, s_{23}\to0$, and the amplitude has two poles, 
$|\mathcal M|^2\sim1/(s_{13}s_{23})\to\infty$. Finally, if only one of the quarks is soft,
then the pole formed is the same as in the collinear limit, and requires no 
separate dedicated soft-quark counterterm \cite{Ellis:1996mzs}. Furthermore, there 
is also the case for the NNLO process $\gamma^*\to q(k_1)\bar q(k_2) q^\prime(k_4)\bar q^\prime(k_5)$ 
in which the $q^\prime(k_4)$ and $\bar q^\prime(k_5)$ become simultaneously soft. In that case, 
the amplitude has a double pole around the relevant invariant, $s_{45}$, which 
appears as $|\mathcal M|^2\sim1/s_{45}^2\to\infty$ \cite{Gehrmann-DeRidder:2005btv}.
...
consists of hard-radiator momenta $\tilde p_\text{hard}$
and the unresolved antenna momenta $p_\text{unresolved}$, which
are integrated over $\Phi_X$. $N_s$
is a numerator built from scalar products, $D_j$ are propagator or Mandelstam
invariants $s_{ij}$'s and $a_j$ are integer powers. An algebraic expansion of the integrand
of an antenna function can generate multiple integrals with the same kinematic
