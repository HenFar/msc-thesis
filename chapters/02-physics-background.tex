...
\begin{equation}\label{eq:bornLevel}
    d\sigma_\text{LO} = \int_m d\sigma^B,\quad d\sigma^B=d\Phi_m(p_1,...,p_m;q)\frac1{S_m}|\mathcal M_m^0(p_1,...,p_m)|^2J_m^{m}(p_1,...,p_m)
\end{equation}
...
\begin{equation}
    d\Phi_m(p_1,...,p_m;q)=\frac{d^{d-1}p_1}{2E_1(2\pi)^{d-1}}...\frac{d^{d-1}p_m}{2E_m(2\pi)^{d-1}}(2\pi)^d\delta^{(d)}(q-p_1-...-p_m).
\end{equation}
...
\begin{equation}
    \begin{gathered}
        d\sigma^R_\text{NLO} = d\Phi_{m+1}(p_1,...,p_{m+1};q)\frac1{S_{m+1}}|\mathcal M_{m+1}^0(p_1,...,p_{m+1})|^2J_m^{(m+1)}(p_1,...,p_{m+1}),\\
        d\sigma^V_\text{NLO} = d\Phi_m(p_1,...,p_m;q)\frac1{S_m}\,2\operatorname{Re}\!\left(\mathcal M_m^1(p_1,...,p_m)\mathcal M_m^{0\,*}(p_1,...,p_m)\right)J_m^{(m)}(p_1,...,p_m).
    \end{gathered}
\end{equation}
...
\begin{equation}
    \begin{gathered}
        d\sigma^{RR}_\text{NNLO} = d\Phi_{m+2}(p_1,...,p_{m+2};q)\frac1{S_{m+2}}|\mathcal M_{m+2}^0(p_1,...,p_{m+2})|^2J_m^{(m+2)}(p_1,...,p_{m+2}),\\
        d\sigma^{RV}_\text{NNLO} = d\Phi_{m+1}(p_1,...,p_{m+1};q)\frac1{S_{m+1}}\,2\operatorname{Re}\!\left(\mathcal M_{m+1}^1(p_1,...,p_{m+1})\mathcal M_{m+1}^{0\,*}(p_1,...,p_{m+1})\right)J_m^{(m+1)}(p_1,...,p_{m+1}),\\
        \begin{aligned}
            d\sigma^{VV}_\text{NNLO} = d\Phi_m(p_1,...,p_m;q)\frac1{S_m}\Big[&2\operatorname{Re}\!\left(\mathcal M_m^2(p_1,...,p_m)\mathcal M_m^{0\,*}(p_1,...,p_m)\right)\\
...
\begin{equation}
    \int_1 d\sigma^S_\text{NLO} = \sum_m d\Phi_m(p_1,...,\tilde p_I,\tilde p_K,...,p_{m+1})\frac1{S_{m+1}}\sum_j\left(\int d\Phi_{X_{ijk}}\,X_{ijk}^0\right)|\mathcal M_m(p_1,...,\tilde p_I,\tilde p_K,...,p_{m+1})|^2J_m^{(m)}(p_1,...,\tilde p_I,\tilde p_K,...,p_{m+1}).
\end{equation}
...
originates entirely from the coupling-renormalisation couterterm.
...
\begin{equation}\label{eq:M4q}
    \begin{gathered}
        \begin{aligned}
            M_{4q}^0 = \frac1{|\mathcal M_2^0|^2}\left(\sum_{q^\prime}|\mathcal M_4^0(1_q,3_{q^\prime},4_{\bar q^\prime},2_{\bar q})|^2+|\mathcal M_4^0(1_q,3_{q},4_{\bar q},2_{\bar q})|^2\right)
        \end{aligned}\\
        \begin{aligned}
...
defined, generally\footnote{\label{footnote:integratedAntenna} This definition
is general for antenna functions that are already cleanly and completely given
in terms of Mandelstam invariants and $\epsilon$. This is only automatically
true for antenna where $l=0$. When $l\geq1$, which happens for all loop
antennae by definition, a prior loop-momentum integration step is necessary.
This step is explored further in Section~\ref{sec:integration}.}, as
...
antennae containing unresolved radiation, is not required. $A_2^0$ also has no
