...
(the differential cross-section) for some $F_n$ infrared-safe quantity or observable, 
for $n$ final-state particles. $F_n$, in this case, is a distribution. The terms 
...
formula has two, $d\sigma^S$ and $d\sigma^T$, which respectively subtract the 
singularities of $d\sigma^{RR}$ at single- and double-unresolved level, and 
$d\sigma^{RV}$ at single-unresolved level \cite{Gehrmann-DeRidder:2005btv}.  
This inadvertently adds another level of
...
notation, as defined in Subsection~\ref{subsec:antennaFuncs}, as,
...
the colour prefactor per colour component: $N$ for the leading order, $1/N$ for subleading, etc., 
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|p{11cm}|}
        ...
        \hline
    \end{tabular}
    \caption{Antenna subtraction formulae for $\sigma^\text{NLO}$ and $\sigma^\text{NNLO}$, in terms of the antenna functions \cite{Gehrmann-DeRidder:2005btv}. The $\sigma^\text{LO}$ row has no antenna-function form: it is the un-normalised Born term itself.}
...
The antenna's own phase-space integral is then rewritten as,
