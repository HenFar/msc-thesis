...
become unresolved collinearly. At NNLO, however, a secondary quark-antiquark pair,
produced by the splitting of an intermediate gluon, can become soft as a unit, giving a
double pole in the pair's invariant. The same final state also has a collinear limit when
the secondary pair itself becomes collinear, $q^\prime\parallel\bar q^\prime$, and a
triple-collinear limit when the pair is additionally collinear with one of the hard partons.
These and all other limits considered throughout this thesis are shown
...
\begin{equation}\label{eq:bornLevel}
    d\sigma_\text{LO} = \int_m d\sigma^B,\quad d\sigma^B=\sum_nd\Phi_n(p_1,...,p_n;q)\frac1{S_n}|\mathcal M_n(p_1,...,p_n)|^2J_m^{(n)}(p_1,...,p_n)
\end{equation}
...
\begin{equation}
    d\Phi_n(p_1,...,p_n;q)=\frac{d^{d-1}p_1}{2E_1(2\pi)^{d-1}}...\frac{d^{d-1}p_n}{2E_n(2\pi)^{d-1}}(2\pi)^d\delta^{(d)}(q-p_1-...-p_n).
\end{equation}
...
becomes,
\begin{equation}
    d\sigma_\text{NLO} = \int_{m+1}\left(d\sigma^R_\text{NLO}-d\sigma^S_\text{NLO}\right)+\int_m\left(d\sigma^V_\text{NLO}+\int_1 d\sigma^S_\text{NLO}\right)=\int_m\left(d\sigma^V_\text{NLO}+\int_1 d\sigma^S_\text{NLO}\right).
\end{equation}
...
This quantity is fully inclusive in the phase space, which means that $J_m^{(n)}=1$
everywhere, independent of the order. For the two-hard-radiator process considered here,
$d\sigma^S_\text{NLO}$ is built from the full real-radiation matrix element and therefore
coincides with $d\sigma^R_\text{NLO}$ pointwise, not only in the unresolved limits
\cite{Gehrmann-DeRidder:2005btv} --- a special property of two-jet production that does not
hold for generic multi-jet observables. With $J_m^{(n)}=1$, the $d\sigma^R_\text{NLO}-d\sigma^S_\text{NLO}$
term therefore vanishes identically rather than integrating to a finite remainder, leaving only
the analytically integrated contribution. Without the jet function in the expression,
the phase space can also be factorised as,
\begin{equation}
...
\begin{equation}
    \begin{aligned}
        d\sigma_\text{NNLO} &= \int_{m+2}\left(d\sigma_\text{NNLO}^{RR}-d\sigma_\text{NNLO}^{S}\right)+\int_{m+1}\left(d\sigma_\text{NNLO}^{RV}-d\sigma_\text{NNLO}^T\right)+\int_m d\sigma_\text{NNLO}^{VV}\\
...
processes with three or more hard partons, beyond those considered in the current text. Their
complete form is written in \cite{Gehrmann-DeRidder:2005btv}.

The iterated mapping for the four-parton system $(i,j,k,l)$ proceeds in two stages,
\begin{equation}
    \begin{gathered}
        (i,j,k)\to(\tilde I,\tilde K)\\
        (\tilde I,\tilde K,l) \to (\tilde{\tilde{I}}, \tilde{\tilde{L}})
    \end{gathered}
\end{equation}

and the three characteristic colour-connection configurations are shown in
Fig.~\ref{fig:colourConnectionNNLOall}.
...
\caption{\centering The three characteristic double-unresolved colour-connection configurations at NNLO: (a) two colour-connected unresolved partons, (b) two adjacent single-unresolved antennae, and (c) two disconnected single-unresolved antennae.}
\label{fig:colourConnectionNNLOall}
\end{figure}

Once again, as for NLO, the massless hadronic $R$-ratio is the jet observable
considered, and hence $J_m^{(n)}=1$. As at NLO, $d\sigma^S_\text{NNLO}$ and $d\sigma^T_\text{NNLO}$
are built from the full real-radiation matrix elements, so the $d\sigma^{RR}_\text{NNLO}-d\sigma^S_\text{NNLO}$
and $d\sigma^{RV}_\text{NNLO}-d\sigma^T_\text{NNLO}$ terms likewise vanish identically for this
observable \cite{Gehrmann-DeRidder:2005btv}. The phase space factorises as,
\begin{equation}
    d\Phi_{m+2}(p_1,...,p_{m+2};q) = d\Phi_m(p_1,...,\tilde p_I,\tilde p_L,...,p_{m+2};q)d\Phi_{X_{ijkl}}(p_i,p_j,p_k,p_l;q=\tilde p_I+\tilde p_L)
\end{equation}
and hence,
\begin{equation}
...
might have as many as one unresolved parton at NLO and two at NNLO.
We also consider 
...
To compute the considered antenna functions, we require the squared matrix element at each
loop order, implicitly normalised with $N(n)$ as described in Eq.~\ref{eq:gehrN}. For the
tree-level, one-loop, and two-loop cases these are given as,
\begin{equation}
    \begin{gathered}
...
\end{equation}
where, for final states admitting more than one colour ordering, flavour assignment, or coupling
configuration, the product of the amplitudes is understood to be a complete process-level contraction,
...
complete that decomposition are discussed in the next subsection \cite{Gehrmann-DeRidder:2005btv}.
...
\begin{itemize}
    \item\label{list:A31Comps} The leading colour component ($N$): these come from planar topologies
    ...
    $A_3^1$. An NNLO leading antenna-set component is
...
\begin{equation}
    ...
    \end{gathered}
\end{equation}
reaching the two components that make up the set $\left\{A_4^0,\tilde A_4^0\right\}$.
...
\end{equation}
leaving the final antenna set $\left\{A_3^1,\tilde A_3^1,\hat A_3^1\right\}$.
...
\begin{equation}\label{eq:a22Amp}
    \begin{gathered}
        M_{q\bar q}^2 =\left(\frac{2\operatorname{Re}\!\left(\mathcal M_2^{0\,\dagger}(1_q,2_{\bar q})\mathcal M_2^2(1_q,2_{\bar q})\right)+\mathcal M_2^{1\,\dagger}(1_q,2_{\bar q})\mathcal M_2^1(1_q,2_{\bar q})}{|\mathcal M_2^0(1_q,2_{\bar q})|^2}\right) = \frac{|\mathcal M_2^{2,[2\times 0]}|^2+|\mathcal M_2^{2,[1\times 1]}|^2}{|\mathcal M_2^0|^2}\\
    ...
    \end{gathered}
\end{equation}
reaching the four components that make up the set
$\left\{A_2^2,\tilde A_2^2,\hat A_2^2,\breve A_2^2\right\}$.
...
expressions. For that, we begin by defining $q\bar q\text{-}\gamma$ coupling amplitude using the Fierz identity,
\begin{equation}
    \mathcal M_4^0(1_q,3_{q^\prime},4_{\bar q^\prime},2_{\bar q}) = ie_qg^2\,\delta_{q_1q_2}\delta_{q_3q_4}\left(\delta_{i_1i_4}\delta_{i_3i_2}-\frac1N\delta_{i_1i_2}\delta_{i_3i_4}\right)\mathcal M_{q\bar q\text{-}\gamma}(1_q,3_q,4_{\bar q},2_{\bar q}) + (1\leftrightarrow3,\,2\leftrightarrow4).
\end{equation}
Using this amplitude, and comparing its structure with Eq.~\ref{eq:M4q}'s, we are able to write,
\begin{equation}
    \begin{gathered}
...
are built, combined, and normalised. As ingredients of the subtraction term
$d\sigma^S$, the antenna functions must then be integrated over the unresolved-parton
phase space. Schematically, as described in Eq.~\ref{eq:subScheme}, this is the
integral,
\begin{equation}
    \int_1 d\sigma^S
\end{equation}
...
unresolved parton, but is excluded here for a different reason: it is the
