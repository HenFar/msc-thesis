...
procedimento automatizado. O pacote \textit{AntCalc} reproduz
...
Chromodynamics (QCD), predictions are systematically improved
...
singularities associated with soft and collinear parton radiation. These singularities
cancel in infrared-safe observables, but the real and virtual terms are separately
divergent and are defined over phase spaces of different multiplicities. Their numerical evaluation
therefore requires a procedure that isolates the singular behaviour and implements
its cancellation explicitly.

Antenna subtraction does this systematically: universal antenna functions describe
unresolved radiation between colour-connected hard partons; they are subtracted locally
from the real-emission contribution and integrated analytically in dimensional
regularisation, making the cancellation of infrared poles against the virtual
...
keeping process-dependent
information separate from the general workflow. It generates the required 
...
integrated antennae agree with existing results in the literature, and their
combined consistency is validated through the massless hadronic \(R\)-ratio up
to NNLO, \(\mathcal{O}(\alpha_s^2)\).
The framework is also extended beyond the massless case through the implementation
of the massive \(A_3^0\) antenna. Future work will extend \textit{AntCalc} to further massive antennae,
