...
In[1]:= A40 = BuildAntenna[A,4,0,Component->Leading];
        ...
        soft3Leading = SeriesCoefficient[A40 /. soft3Rules,
            {lambda, 0, -2}] // Simplify;

        A30142 = BuildAntenna[A,3,0] /. {s13 -> s14, s23 -> s24,
            q2 -> s124};
...
In[1]:= A40 = BuildAntenna[A,4,0,Component->Leading];
        ...
        col13Rules = {s13 -> lambda, s12 -> z sP2, s23 -> (1-z) sP2,
            ...
            Epsilon -> 0};
        col13Leading = SeriesCoefficient[Together[A40 /. col13Rules],
            {lambda, 0, -1}] // FullSimplify;

        A30P42 = BuildAntenna[A,3,0] /. {s12 -> sP2, s13 -> sP4,
            s23 -> s24, q2 -> sP2 + sP4 + s24, Epsilon -> 0};
...
In[1]:= A40 = BuildAntenna[A,4,0,Component->Leading];
        ...
        col34Rules = {s34 -> lambda, s13 -> z s1P, s14 -> (1-z) s1P,
            ...
            Epsilon -> 0};
        col34Leading = SeriesCoefficient[Together[A40 /. col34Rules],
            {lambda, 0, -1}] // FullSimplify;
...
In[1]:= tA40 = BuildAntenna[A,4,0,Component->Subleading];
        ...
        soft3Rules = {s13 -> lambda s13, s23 -> lambda s23,
            s34 -> lambda s34, s134 -> s14, s234 -> s24, q2 -> s124};
        soft3Leading = SeriesCoefficient[tA40 /. soft3Rules,
            {lambda, 0, -2}] // Simplify;
...
In[1]:= tA40 = BuildAntenna[A,4,0,Component->Subleading];
        ...
        col13Rules = {s13 -> lambda, s12 -> z sP2, s23 -> (1-z) sP2,
            ...
            s234 -> (1-z) sP2 + s24 + (1-z) sP4, q2 -> sP2 + sP4 + s24,
            Epsilon -> 0};
        col13Leading = SeriesCoefficient[Together[tA40 /. col13Rules],
            {lambda, 0, -1}] // FullSimplify;
...
In[1]:= B40 = BuildAntenna[B,4,0];
        ...
        col34Rules = {s34 -> lambda, s13 -> z s1P, s14 -> (1-z) s1P,
            ...
            q2 -> s12 + s1P + sP2};
        B40Col34Leading = SeriesCoefficient[Together[B40 /. col34Rules],
            {lambda, 0, -1}] // FullSimplify;

        A301P2 = BuildAntenna[A,3,0] /. {s13 -> s1P, s23 -> sP2,
            q2 -> s12 + s1P + sP2};

        Pqqbar = (z^2 + (1-z)^2 - Epsilon)/(1 - Epsilon);
        spinCorrelationB40 = 4 s12 (z-1) z/(s1P sP2 (Epsilon-1));

        FullSimplify[B40Col34Leading - Pqqbar A301P2
            - spinCorrelationB40]
\end{lstlisting}
\texttt{Out[1]:=}
    \begin{equation*}
...
In[1]:= B40 = BuildAntenna[B,4,0];
        ...
        doubleSoftRules = {s13 -> lambda s13, s14 -> lambda s14,
            ...
            Epsilon -> 0};
        B40DoubleSoftLeading = SeriesCoefficient[
            Together[B40 /. doubleSoftRules], {lambda, 0, -4}]
            // FullSimplify;

        S12qqbar = 2/s34^2 (s12 s34/((s13+s14)(s23+s24))
            - (s13 s24 - s14 s23)^2/((s13+s14)^2 (s23+s24)^2));
...
In[1]:= B40 = BuildAntenna[B,4,0];
        ...
        B40Triple134Leading = SeriesCoefficient[
            Together[B40 /. tripleCollinearRules134], {lambda, 0, -2}]
            // FullSimplify;

        A20P2 = A20 /. q2 -> sP2 // FullSimplify;

        PNonIdent134 = -((1-Epsilon) + 2 r13/r34)/(r13+r14+r34)^2
            - 2 (x (r13+r14+r34) - (1-w) r13)^2
                /(r34^2 (r13+r14+r34)^2 (1-w)^2)
            + 1/(r34 (r13+r14+r34))
                ((1+x^2+(x+w)^2)/(1-w) - Epsilon (1-w));
...
In[1]:= C40 = BuildAntenna[C,4,0];
        ...
        C40Triple234Leading = SeriesCoefficient[
            Together[C40 /. tripleCollinearRules234], {lambda, 0, -2}]
            // FullSimplify;
        ...
        PIdent234 = -(2 r23/r34 + 2)/S234^2
            - x (1+x^2)/(2 r23 r34 (1-y) (1-w))
            + 1/(r34 S234) ((1+x^2)/(1-y) + 2 x/(1-w))
            + (-(2 r34/r23 + 2)/S234^2
            - x (1+x^2)/(2 r34 r23 (1-w) (1-y))
            + 1/(r23 S234) ((1+x^2)/(1-w) + 2 x/(1-y)));
...
In[1]:= mA30 = BuildAntenna[A,3,0,quarkMass->mQ];
        ...
        mA30SoftLeading = SeriesCoefficient[Together[mA30 /. softRules],
            {lambda, 0, -2}] // FullSimplify;

        massiveEikonal = 2 s12/(s13 s23) - 2 mQ^2/s13^2 - 2 mQ^2/s23^2;
...
In[1]:= mA30 = BuildAntenna[A,3,0,quarkMass->mQ];
        ...
        quasi13Rules = {s13 -> lambda r13, s12 -> z sP2,
            ...
            Epsilon -> 0};
        mA30Quasi13Leading = SeriesCoefficient[
            Together[mA30 /. quasi13Rules], {lambda, 0, -1}]
            // FullSimplify;

        PQg = (1 + z^2)/(1 - z) - 2 muQg2;
